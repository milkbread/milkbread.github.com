			...
			for (var i=0;i<data.arcs.length;i++){
				...
				for (var j=0; j<data.arcs[i].length;j++){
					...
					if((arc_id in col_arcs_)===false){
						...
						col_arcs_[arc_id].members.push({id:d.id, direction:sign, name:d.properties.GEN})
					}
				}
			}
		}
...
function getGeometries(geometries, col_arcs_){
	...
	geometries.forEach(function(d){
		if(d.type==='Polygon'){
			...
			polygons.push({type:'Polygon', coordinates: a, id:d.id, clipped:d.clipped, properties: d.properties, neighbors: members, arcs:d.arcs})
		}
...
function getArcsOfNeighbores(features, col_arcs_){
	...
	var feat_members = features.map(function(feature){
		...
		if(feature.type==='Polygon'){
			readPolygon2({arcs:feature.arcs}, feature.id); 
			//console.log(members)
		}
		else if(feature.type==='MultiPolygon'){
			feature.arcs.map(function(e,i){return readPolygon2({arcs:feature.arcs[i]},feature.id)}); 
			//console.log(members)
		}	
		return members;
	});
...
function findLongestArc2(neigh_arcs, arcCollection_){
	var list = neigh_arcs.map(function(a){return a;})
